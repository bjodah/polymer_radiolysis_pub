...
MK_${TOKEN}=--n0 ${N0} -n "${N_LEVELS}" -a "[0,1,2,6,24,120,720]" -p "[0,1,10,100]" -o "[0,1,2]" -l "[0,1,10,100,1000,10000]" \
...
${crn(scal=scal)}: mk_radsys rad-rxns.json rad-subst.json
	mkdir -p ${TOKEN}-res/
	./$< --output $@ $(MK_${TOKEN}) --primary-data '${json.dumps(dict(cases.scals[scal], MW_kDa=float(MW_kDa), chain_rad_chain_rad_M_s=2.5e7*(float(MW_kDa)/410.0)**-0.6, t_half_loop_closure_intra_s_94kDa=float(t_half_loop_closure_intra_s_94kDa)))}'
