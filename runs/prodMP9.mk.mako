...
peroxy_inter_M_s=float(peroxy_factor) * alkyl_inter_M_s
...
${crn(scal=scal)}: mk_radsys rad-rxns.json rad-subst.json
	mkdir -p ${TOKEN}-res/
	./$< --output $@ $(MK_${TOKEN}) --primary-data '${json.dumps(dict(cases.scals[scal], MW_kDa=float(MW_kDa), alkyl_inter_M_s=alkyl_inter_M_s, peroxy_inter_M_s=peroxy_inter_M_s, t_half_loop_closure_intra_s_94kDa=float(t_half_loop_closure_intra_s_94kDa)))}'
