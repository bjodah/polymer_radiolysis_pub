...
MK_${TOKEN}=--n0 ${N0} -n "${N_LEVELS}" -a "[0,1,10,100]" -p "[0,1,10,100]" -o "[0,1,2]" -l "[0,1,10,100,1000]" \
		--reactions rad-rxns.json --substances rad-subst.json
SOLVE_${TOKEN}= --abstol abstol-${TOKEN}.json --settings '{"rtol": 1e-10, "mxsteps": 25000}' --duration 0.0  --varied varied-${TOKEN}.json --npoints 2
...
%endfor
${tar(i, j, k)}: ${' '.join([res(i, j, k, _l) for _l in range(len(concs))])}
	tar cJf $@ $^
