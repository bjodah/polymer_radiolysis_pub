...
%for ifreq in range(len(cases.freqs)):
varied-${TOKEN}-${ifreq}.json: mk_varied.py
	./mk_varied.py --outfile $@   --tot-dose-Gy 40e3 --count-wait "[]" --dr-pulse-Gy-s 32e6 --t-pulse-s 5e-6 --f-pulse-Hz ${cases.freqs[ifreq]}
%endfor
...
${res(icrn=icrn, iint=iint)}: ${crn(icrn=icrn)} solve_ivp varied-${TOKEN}-${ifreq}.json abstol-${TOKEN}.json
	./solve_ivp -i $< -o $@ $(SOLVE_ARGS_${TOKEN}) --varied varied-${TOKEN}-${ifreq}.json \
...
--vmin 1e-15 --vmax ${10.0 * cases.mass_fracs[imf]*998/(float(MW_kDa)*1e3)} --varied varied-${TOKEN}-${ifreq}.json --savemov $@
