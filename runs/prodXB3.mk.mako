...
MK_${TOKEN}=--n0 ${N0} -n "${N_LEVELS}" -a "[0,1,2,6,24,120,720,5040]" -p "[0,1,2,4,8]" -o "[0,1,2]" -l "[0,1,10,100,1000,10000]" \
...
%for itpuls in range(len(cases.tpulses_s)):
varied-${TOKEN}-${itpuls}.json: mk_varied.py
	./mk_varied.py --outfile $@   --tot-dose-Gy 40e3 --count-wait "[]" --dr-pulse-Gy-s 32e5 --t-pulse-s ${cases.tpulses_s[itpuls]} --f-pulse-Hz 40.0
%endfor
...
%for iint, (imf, iatmo, itpuls) in enumerate(cases.int_indices):
${res(icrn=icrn, iint=iint)}: ${crn(icrn=icrn)} solve_ivp varied-${TOKEN}-${itpuls}.json abstol-${TOKEN}.json
	./solve_ivp -i $< -o $@ $(SOLVE_ARGS_${TOKEN}) --varied varied-${TOKEN}-${itpuls}.json \
...
--vmin 1e-15 --vmax ${10.0 * cases.mass_fracs[imf]*998/(float(MW_kDa)*1e3)} --varied varied-${TOKEN}-${itpuls}.json --savemov $@
